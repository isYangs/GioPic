...
const requestData = {
  ...
  uploadImage: (type: ProgramType, url: string, token: string, data: UploadData) => {
    switch (type) {
      ...
      case 'lskyPro':
        return requestData._uploadLskyProImage(url, token, data)
      default:
        return Promise.reject(new Error('不支持的图床类型'))
    }
  },

  _uploadLskyImage: (url: string, token: string, data: UploadData) => {
    const req = createRequest(url, token, {
  ...
  _uploadLskyProImage: (url: string, token: string, data: UploadData) => {
    const req = createRequest(url, token, {
      'Content-Type': 'multipart/form-data',
    })
    return req('/api/v1/upload', 'post', data)
