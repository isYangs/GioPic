...
import { ImgLinkFormatTabsOption } from '~/components/SettingsPanel/utils';
...
export const useAppStore = defineStore(
    ...
    () => {
        ...
        async function getApiUrlTitle(): Promise<boolean> {
            ...
            const title = doc.querySelector('title')?.textContent?.split('-')?.[0] || doc.querySelector('title')?.textContent?.split(' ')?.[0] || '未知图床';
    ...
    },
    {
        persist: {
            key: 'APP_SETTING',
            paths: ['bgImgUrl', 'isSettingsDrawer', 'apiUrl', 'token', 'apiUrlTitle', 'strategies', 'strategiesVal', 'imgLinkFormatVal', 'imgLinkFormatTabs', 'recordSavePath', 'lastCallTime', 'isImgListDelDialog', 'isUploadRecordDelDialog'],
        },
    }
);
