...
const settingsFormRules: FormRules = {
  ...
  strategiesVal: createFormRule(() =>
    strategiesDisabled.value === false ? validate.validateStrategiesVal(settingsFormModel.value.strategiesVal) : true,
  ),
...
// 处理同步存储策略的异步函数
const handleSyncStrategies = limitFunctionCallFrequency(
  async () => {
    strategiesDisabled.value = true
