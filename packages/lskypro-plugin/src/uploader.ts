...
async function uploadToSingleStrategy(options: LskyUploadParams, strategyId?: string | number): Promise<UploaderResponse> {
  ...
  if (useDefaultPermission) {
    formData.append('is_public', (defaultPermission ?? 1).toString())
    options.disablePermissionSelect = true
  }
  else if (permission !== undefined) {
    formData.append('is_public', permission.toString())
  }

  logger.info(`发送上传请求: ${api}/api/v2/upload${strategyId ? ` (策略ID: ${strategyId})` : ''}`)

  const { data: res } = await request<LskyUploadResponse>({
    url: `${api}/api/v2/upload`,
    method: 'POST',
    headers: {
      ...formData.getHeaders(),
